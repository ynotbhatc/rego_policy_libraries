...
# CIS Rocky Linux 8 Benchmark v2.0.0 - Section 4.1: Configure System Accounting (auditd)
...
config_violations contains "CIS 4.1.1.4: audit_backlog_limit not configured (should be 8192 or higher)" if {
...
config_violations contains "CIS 4.1.2.3: admin_space_left_action not appropriate (should be single or halt)" if {
...
rules_violations contains sprintf("CIS 4.1.3.x: Missing audit rule for: %s", [rule_desc]) if {
...
# CIS 4.1.4.4: Audit config file permissions
