...
# Section 3: Logging Monitoring and Auditing (Enhanced for PostgreSQL 15)
logging_violations := [v |
	arrays := [
		...
		["3.27: Ensure 'log_recovery_conflict_waits' is enabled" | not log_recovery_conflict_waits_enabled],
		["3.28: Ensure 'log_parameter_max_length' is set appropriately" | not log_parameter_max_length_set]
	]
	v := arrays[_][_]
]
...
log_recovery_conflict_waits_enabled if {
	input.postgresql.logging.log_recovery_conflict_waits == "on"
}

log_parameter_max_length_set if {
	log_param_max_length := input.postgresql.logging.log_parameter_max_length
	log_param_max_length >= 0
	log_param_max_length <= 1024
}

# Section 4: User Access and Authorization (Enhanced for PostgreSQL 15)
user_access_violations := [v |
	arrays := [
		...
		["4.31: Ensure predefined roles are used appropriately" | not predefined_roles_appropriate],
		["4.32: Ensure security definer functions are reviewed" | not security_definer_functions_reviewed]
	]
	v := arrays[_][_]
]
...
predefined_roles_appropriate if {
	predefined_roles := input.postgresql.predefined_roles
	role_usage := predefined_roles.usage
	role_usage.pg_read_all_data_justified == true
	role_usage.pg_write_all_data_justified == true
	role_usage.pg_read_all_settings_controlled == true
}

security_definer_functions_reviewed if {
	security_definer_functions := input.postgresql.security_definer_functions
	reviewed_functions := [f | f := security_definer_functions[_]; f.security_reviewed == true]
	count(reviewed_functions) == count(security_definer_functions)
}

# Section 5: Connection and Login (Enhanced for PostgreSQL 15)
connection_ssl_violations := [v |
	arrays := [
		...
		["5.25: Ensure SSL passphrase command is configured securely" | not ssl_passphrase_command_secure],
		["5.26: Ensure ssl_library is set appropriately" | not ssl_library_appropriate]
	]
	v := arrays[_][_]
]
...
ssl_passphrase_command_secure if {
	ssl_config := input.postgresql.ssl
	passphrase_command := ssl_config.ssl_passphrase_command
	passphrase_command != ""
	not contains(passphrase_command, "echo")
	not contains(passphrase_command, "cat")
}

ssl_library_appropriate if {
	ssl_config := input.postgresql.ssl
	ssl_library := ssl_config.ssl_library
	ssl_library == "openssl"
}

# Section 6: PostgreSQL Settings (Enhanced for PostgreSQL 15)
postgresql_settings_violations := [v |
	arrays := [
		...
		["6.41: Ensure 'wal_init_zero' is enabled for security" | not wal_init_zero_enabled],
		["6.42: Ensure 'wal_recycle' is disabled for security" | not wal_recycle_disabled]
	]
	v := arrays[_][_]
]
...
wal_init_zero_enabled if {
	wal_config := input.postgresql.wal_settings
	wal_config.wal_init_zero == "on"
}

wal_recycle_disabled if {
	wal_config := input.postgresql.wal_settings
	wal_config.wal_recycle == "off"
}

# Section 7: Replication (Updated for PostgreSQL 15)
