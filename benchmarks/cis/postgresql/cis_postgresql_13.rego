...
postgresql_settings_violations := [v |
	arrays := [
		...
		["6.25: Ensure 'wal_keep_segments' is configured correctly" | not wal_keep_segments_configured],
...
wal_keep_segments_configured if {
	wal_config := input.postgresql.wal_settings
	wal_keep_segments := wal_config.wal_keep_segments
...
replication_violations := [v |
	arrays := [
		...
		["7.2: Ensure the backup is done using the pg_dump utility" | not backup_using_pg_dump],
		...
		["7.11: Ensure failover procedures are documented and tested" | not failover_procedures_tested],
		["7.12: Ensure point-in-time recovery is configured" | not point_in_time_recovery_configured],
		["7.13: Ensure backup encryption is configured" | not backup_encryption_configured],
		["7.14: Ensure backup retention policy is enforced" | not backup_retention_enforced]
	]
	v := arrays[_][_]
]
...
backup_using_pg_dump if {
	backup_methods := input.postgresql.backup_methods
	"pg_dump" in backup_methods
	"pg_dumpall" in backup_methods
}
...
standby_servers_configured if {
	...
	standby_config.recovery_conf_secured == true
}
...
failover_procedures_tested if {
	failover := input.postgresql.failover
	failover.procedures_documented == true
	failover.regularly_tested == true
	failover.automated_failover_configured == true
}

point_in_time_recovery_configured if {
	pitr := input.postgresql.point_in_time_recovery
	pitr.wal_archiving_enabled == true
	pitr.base_backups_automated == true
	pitr.recovery_procedures_documented == true
}

backup_encryption_configured if {
	backup_encryption := input.postgresql.backup_encryption
	backup_encryption.enabled == true
	backup_encryption.strong_encryption == true
	backup_encryption.key_management_secure == true
}

backup_retention_enforced if {
	backup_retention := input.postgresql.backup_retention
	backup_retention.policy_defined == true
	backup_retention.automated_cleanup == true
	backup_retention.compliance_verified == true
}
...
unnecessary_extensions_disabled if {
	...
	approved_extensions := ["pgaudit", "pg_stat_statements", "uuid-ossp", "citext"]
