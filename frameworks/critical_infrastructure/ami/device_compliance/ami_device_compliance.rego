...
firmware_violation_msgs(device) := msgs if {
	msgs := array.concat(
		[sprintf(
			"Device %v: Firmware %v is below minimum required version %v",
...
cert_violation_msgs(device) := msgs if {
	...
	expiring_msgs := [sprintf(
		"Device %v: WARNING — Device certificate expires within 30 days (%v)",
		[device.device_id, device.certificate.expiry_date],
	) |
		cert_not_expired(device)
		not cert_not_expiring_soon(device)
	]
	weak_algo_msgs := [sprintf(
		"Device %v: Certificate uses weak algorithm: %v",
...
key_violation_msgs(device) := msgs if {
	rotation_msgs := [sprintf(
		"Device %v: Key rotation overdue — last rotation: %v (>90 days ago)",
		[device.device_id, device.key_management.last_rotation_date],
	) |
		not key_rotation_current(device)
	]
	storage_msgs := [sprintf(
		"Device %v: Keys not stored in secure hardware (TPM/secure element/HSM)",
		[device.device_id],
	) |
...
comms_violation_msgs(device) := msgs if {
	weak_cipher_msgs := [sprintf(
		"Device %v: Prohibited encryption algorithms in use",
		[device.device_id],
	) |
		not strong_encryption_only(device)
	]
	no_mutual_auth_msgs := [sprintf(
		"Device %v: Mutual authentication with head-end not enabled",
		[device.device_id],
	) |
		not mutual_auth_enabled(device)
	]
	not_encrypted_msgs := [sprintf(
		"Device %v: Communications not encrypted with approved AES mode",
		[device.device_id],
	) |
...
tamper_violation_msgs(device) := msgs if {
	tamper_msgs := [sprintf(
		"Device %v: CRITICAL — Physical tamper detected! Immediate investigation required",
		[device.device_id],
	) |
		not no_tamper_detected(device)
	]
	detection_msgs := [sprintf(
		"Device %v: Tamper detection hardware not enabled or not operational",
		[device.device_id],
	) |
